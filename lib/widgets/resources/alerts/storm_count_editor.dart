...
class StormCountEditor extends StatelessWidget {

  static const double height = 290;
  const StormCountEditor({ Key? key }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: <Widget>[
      const SectionTitle("Storm Count"),
      KrWidgets.height10,
      logic.storm.build((_, val) => buttons(val),),
      KrWidgets.height10,
      insertTile(logic, stage),
      KrWidgets.divider,
      logic.storm.build((_, val) => doubleTile(val, logic),),
      KrWidgets.divider,
      logic.storm.build((_, val) => resetTile(val, logic),),
      KrWidgets.height10,
    ],);
  }
  ...
  Widget doubleTile(int val, Logic logic) => ListTile(
    ...
    onTap: val == 0 ? null : () => logic.storm.set(val * 2),
    enabled: val > 0,
  );
  ...
  Widget resetTile(int val, Logic logic) => ListTile(
    ...
    onTap: val == 0 ? null : () => logic.storm.set(0),
    enabled: val > 0,
  );
...
void _edit(StageData stage, Logic logic) => stage.showAlert(
  InsertAlert(
    ...
    onConfirm: (val) => logic.storm.set(int.parse(val)),
  ),
