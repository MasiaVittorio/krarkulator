...
class CopiedCountEditor extends StatelessWidget {

  static const double height = 290;
  const CopiedCountEditor({ Key? key }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: <Widget>[
      const SectionTitle("Copied Count"),
      KrWidgets.height10,
      logic.copiedCount.build((_, val) => buttons(val),),
      KrWidgets.height10,
      insertTile(logic, stage),
      KrWidgets.divider,
      logic.copiedCount.build((_, val) => doubleTile(val, logic),),
      KrWidgets.divider,
      logic.copiedCount.build((_, val) => resetTile(val, logic),),
      KrWidgets.height10,
    ],);
  }
  ...
  Widget doubleTile(int val, Logic logic) => ListTile(
    ...
    onTap: val == 0 ? null : () => logic.copiedCount.set(val * 2),
    enabled: val > 0,
  );
  ...
  Widget resetTile(int val, Logic logic) => ListTile(
    ...
    onTap: val == 0 ? null : () => logic.copiedCount.set(0),
    enabled: val > 0,
  );
...
void _edit(StageData stage, Logic logic) => stage.showAlert(
  InsertAlert(
    ...
    onConfirm: (val) => logic.copiedCount.set(int.parse(val)),
  ),
    ...
    return SizedBox(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(8),
          onTap: disabled ? null : (){
            logic.copiedCount.value += value;
            logic.copiedCount.refresh();
          },
          child: Row(children: [
