...
class Spell {
  ...
  static Spell fromJson(Map<String, dynamic> json) => Spell(
        json["name"],
        <MtgColor?, int>{
          for (final c in MtgColor.values) c: json["manaCost"][c.name] ?? 0,
          null: json["manaCost"][null] ?? 0,
        },
        <MtgColor?, int>{
          for (final c in MtgColor.values) c: json['manaProduct'][c.name] ?? 0,
  ...
  Map<String, dynamic> get toJson => {
        "name": name,
        "manaCost": <String, int>{
          for (final c in MtgColor.values) c.name: manaCost[c] ?? 0,
          _clrless: manaCost[null] ?? 0,
        },
        "manaProduct": <String, int>{
          for (final c in MtgColor.values) c.name: manaProduct[c] ?? 0,
          _clrless: manaProduct[null] ?? 0,
        },
        "treasuresProduct": treasuresProduct,
        "board_product": <String, int>{
          for (final e in boardProduct.entries) e.key.name: e.value,
        },
        "storm": storm,
      };
